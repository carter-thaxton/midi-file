...
function writeTrack(w, track) {
  var t = new Writer()
  var i, len = track.length
  for (i=0; i < len; i++) {
    writeEvent(t, track[i])
  }
...
function writeEvent(w, event) {
  ...
  switch (type) {
    ...
    // channel events
    case 'noteOff':
      var noteByte = event.velocity == 0 ? 0x90 : 0x80
      w.writeUInt8(noteByte | event.channel)
      w.writeUInt8(event.noteNumber)
      w.writeUInt8(event.velocity)
      break;

    case 'noteOn':
      w.writeUInt8(0x90 | event.channel)
      w.writeUInt8(event.noteNumber)
      w.writeUInt8(event.velocity)
      break;

    case 'noteAftertouch':
      w.writeUInt8(0xA0 | event.channel)
      w.writeUInt8(event.noteNumber)
      w.writeUInt8(event.amount)
      break;

    case 'controller':
      w.writeUInt8(0xB0 | event.channel)
      w.writeUInt8(event.controllerType)
      w.writeUInt8(event.value)
      break;

    case 'programChange':
      w.writeUInt8(0xC0 | event.channel)
      w.writeUInt8(event.programNumber)
      break;

    case 'channelAftertouch':
      w.writeUInt8(0xD0 | event.channel)
      w.writeUInt8(event.amount)
      break;

    case 'pitchBend':
      w.writeUInt8(0xE0 | event.channel)
      var lsb14 = (event.value & 0x7F)
      var msb14 = (event.value >> 7) & 0x7F
      w.writeUInt8(lsb14)
      w.writeUInt8(msb14)
    break;
